	...
	<div class="flex flex-wrap mt-4 p-4 transition-all  pb-36">
		...
		{:then result}
			...
			{:else}
				<!-- error box  -->
				<div
					class="grow sm:grow-0 order-first sm:order-none w-60 h-48 rounded-xl border-2 p-4 m-2 border-slate-400 bg-slate-200 flex flex-col items-center justify-center transition-all"
				>
					...
					<span
						...
						class="material-icons-round text-slate-600 rounded-full p-2 bg-slate-300 hover:text-slate-700 hover:cursor-pointer hover:rotate-45 transition-all "
						>refresh</span
					>
				</div>
			{/if}

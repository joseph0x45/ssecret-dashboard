...
<svelte:head>
	<title>SuperSecret | Dashboard</title>
</svelte:head>
<!-- main content -->
<div class="w-full h-full z-0 overflow-y-auto bg-slate-100 dark:bg-slate-800   ">
	...
	<div class="flex flex-wrap mt-4 p-4 transition-all  pb-36">
		...
		{#await usersProjects}
			<!-- loader box -->
			<div
				class="grow sm:grow-0 order-first sm:order-none w-60 h-48 rounded-xl border-2 p-4 m-2 border-slate-400 bg-slate-200 dark:border-slate-700 dark:bg-slate-800 flex flex-col items-center justify-center transition-all animate-pulse "
			>
				<svg
					...
					stroke-width="1.5"
					stroke="currentColor"
					class="w-12 h-12 animate-spin text-slate-700 dark:text-slate-300"
				>
					<path
		...
		{:then result}
			...
			{:else}
				...
				>
					<span class="material-icons-round text-slate-600 dark:text-slate-400 text-6xl"
						>warning_amber</span
					>
					<span class="text-slate-400 dark:text-slate-300"
						>Couldn't load projects, try to refresh</span
					>
					<!-- svelte-ignore a11y-click-events-have-key-events -->
					<span
						on:click={fetchProjects}
						class="material-icons-round text-slate-600 dark:text-slate-200 rounded-full p-2 bg-slate-300 dark:bg-slate-500 hover:text-slate-700 dark:hover:text-slate-100 hover:cursor-pointer hover:rotate-45 transition-all "
						>refresh</span
					>
				</div>
				{/if}
		...
		<div
			class={`${
				showForm ? '' : 'hidden'
			} grow sm:grow-0 order-first sm:order-none w-60 h-48 rounded-xl border-2 p-4 m-2 border-slate-400 dark:border-slate-600 bg-slate-200 dark:bg-slate-700 flex flex-col justify-center transition-all `}
		>
			<input
				bind:value={projectName}
				type="text"
				placeholder="New project name"
				class="rounded-lg px-2 w-full h-8 text-[15px] bg-slate-100 dark:bg-slate-600 focus:outline-none focus:border focus:border-slate-300 dark:focus:border-slate-500 dark:caret-slate-300 dark:text-slate-300"
			/>
			<button
				on:click={createProject}
				class="rounded-full text-slate-500 hover:text-slate-600 mx-2 mt-4 p-1 px-2 bg-slate-300 hover:border hover:border-slate-400 dark:hover:text-slate-400 dark:bg-slate-600 dark:hover:border-slate-500 transition-all "
				>Create
			</button>
			<button
				on:click={toggleShow}
